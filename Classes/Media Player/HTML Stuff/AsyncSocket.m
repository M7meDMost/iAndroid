...
- (BOOL)connectToHost:(NSString*)hostname onPort:(UInt16)port error:(NSError **)errPtr
{
	if(theDelegate == NULL)
	{
		//[NSException raise:AsyncSocketException format:[NSString stringWithFormat:@"Attempting to connect without a delegate. Set a delegate first."]];
	}

	if(theSocket != NULL || theSocket6 != NULL)
	{
		//[NSException raise:AsyncSocketException format:[NSString stringWithFormat:@"Attempting to connect while connected or accepting connections. Disconnect first."]];
	}
